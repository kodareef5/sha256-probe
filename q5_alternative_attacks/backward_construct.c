...
static uint32_t Sig1_tab[256];  /* Sig1_tab[e] = Sigma1(e) */
...
int main(void) {
    ...
    printf("M[0]=0x%02x, fill=0xff, MSB kernel\n\n", M0);
    ...
    if (state1[0] != state2[0]) {
        printf("ERROR: da56 != 0 for M[0]=0x%02x\n", M0);
        return 1;
    }
    ...
    coll_t *bf_colls = (coll_t *)malloc(512 * sizeof(coll_t));
    int bf_count = brute_force_search(bf_colls, 512);
        ...
        reduction(+:n_coll_global,n_de61_hits_global,n_triples_global)
    {
        ...
        uint32_t Ch1_tab[256], Ch2_tab[256];
