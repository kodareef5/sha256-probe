/*
 * bitserial_dp.c — Measure bit-serial automaton width at N=8
 *
 * Enumerate all 2^{4N} message combos. For each, check collision bit by bit.
 * Count survivors at each bit prefix depth.
 * Width = survivors(k) / 2^{4*(N-1-k)} = effective automaton states at bit k.
 * GPU laptop claims width = #collisions at every bit. Verify this.
 *
 * gcc -O3 -march=native -o bitserial_dp bitserial_dp.c -lm
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
int main(){
    ...
    uint32_t fills[]={MASK,0};int found=0;
    for(int fi=0;fi<2&&!found;fi++){
        for(uint32_t m0=0;m0<=MASK&&!found;m0++){
            ...
            if(state1[0]==state2[0]){printf("N=%d M[0]=0x%x fill=0x%x\n",N,m0,fills[fi]);found=1;}
        }
    }
    if(!found){printf("No candidate\n");return 1;}

    printf("\nBit-serial automaton width measurement\n");
    printf("For each of 2^%d configs: check output diff bit by bit\n\n",4*N);
        ...
        for(uint32_t w58=0;w58<(1U<<N);w58++){
            ...
            for(uint32_t w59=0;w59<(1U<<N);w59++){
                ...
                for(uint32_t w60=0;w60<(1U<<N);w60++){
                    ...
                    /* Compute combined output diff */
                    uint32_t dm=0;
                    for(int r=0;r<8;r++) dm|=(fa[r]^fb[r]);

                    /* Count survivors at each bit depth */
                    for(int k=0;k<N;k++){
                        if(dm&(1U<<k)) break;
                        survivors[k]++;
                    }
    ...
    printf("\n=== Bit-Serial Automaton Width at N=%d ===\n",N);
    printf("Search: 2^%d = %u, Collisions: %d\n\n",4*N,1U<<(4*N),n_coll);
    printf("Bit  Survivors      log2    Eff.Width\n");
    for(int k=0;k<N;k++){
        double lg=survivors[k]>0?log2((double)survivors[k]):0;
        /* Width = survivors / (remaining search space) = survivors / 2^{4(N-1-k)} */
        double rem = (k<N-1) ? pow(2.0, 4.0*(N-1-k)) : 1.0;
        printf("  %d  %12llu  %7.2f  %10.1f\n",k,(unsigned long long)survivors[k],lg,survivors[k]/rem);
    }
    printf("\nIf Eff.Width = %d at all bits → constant-width automaton → O(N*%d) solver\n",n_coll,n_coll);
    printf("Time: %.1fs\n",el);
    return 0;
}
