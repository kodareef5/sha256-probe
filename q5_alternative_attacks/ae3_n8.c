...
#define MASK ((1U<<N)-1)
#define MSB (1U<<(N-1))
#define NV (8*N)  /* 64 vars at N=8 */       /* 32 variables: W1[57..60] + W2[57..60] × N bits.
                        * Cascade constraints link them via GF(2). */
#define MAXPOOL 131072 /* 4M states */ /* 2M max states — need room for round 60 expansion */
...
int main() {
    ...
    for (int i = 0; i < nA && i < 16; i++) {
        ...
        for (int r = 0; r < 8; r++) {
            ...
            if (sym_val != bf[r]) {
                printf("  FAIL: state %d (w=%x) reg %d: symbolic=0x%llx brute=0x%x\n",
    ...
    for (int rnd = 0; rnd < 7; rnd++) {
        ...
        /* DEBUG: print W2 forms for state 0 */
        if (rnd==0 && nA>0) {
            printf("    W2 forms: ");
            for(int b=0;b<N;b++) printf("(%llx,%d)",PA[0].w[b].m,PA[0].w[b].c);
            printf("\n");
        }
        /* Process msg2 */
        nA = do_one_round(nA, rnd, 1);

        printf("    after msg2: %d states\n", nA);
        /* DEBUG: print register a diffs */
        if (rnd == 0 && nA > 0) {
            printf("    DEBUG rnd 57: a1[0..7]=");
            for(int b=0;b<N;b++){AF f=g2_res(&PA[0].sys,PA[0].r1[0][b]);printf("(%llx,%d)",f.m,f.c);}
            printf("\n    a2[0..7]=");
            for(int b=0;b<N;b++){AF f=g2_res(&PA[0].sys,PA[0].r2[0][b]);printf("(%llx,%d)",f.m,f.c);}
            printf("\n    diff=");
            for(int b=0;b<N;b++){AF d=af_xor(PA[0].r1[0][b],PA[0].r2[0][b]);AF dr=g2_res(&PA[0].sys,d);printf("(%llx,%d)",dr.m,dr.c);}
            printf("\n");
        }
