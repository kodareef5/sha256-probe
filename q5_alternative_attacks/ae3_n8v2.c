...
#define MASK ((1U<<N)-1)
#define MSB (1U<<(N-1))
#define NV 32       /* 32 variables: W1[57..60] + W2[57..60] × N bits.
    ...
    uint32_t w57_start = argc>3 ? atoi(argv[3]) : 0; uint32_t w57_end = argc>4 ? atoi(argv[4]) : (1U<<N); for (uint32_t w57 = w57_start; w57 < w57_end; w57++) {
        ...
        for (uint32_t w58 = 0; w58 < (1U << N); w58++) {
            ...
            for(int rnd=2;rnd<7 && nA>0;rnd++) {
                ...
                if(is_free) {
                    for(int i=0;i<nA;i++) for(int b=0;b<N;b++)
                        PA[i].w[b]=af_var((2+(rnd-2))*N+b);
                } else {
                    for(int i=0;i<nA;i++) {
                        uint32_t ww2[4]={w2_57,w2_58,0,0};
                        for(int b=0;b<N;b++){
                            AF f=g2_res(&PA[i].sys,af_var(2*N+b));
                            if(af_is_const(f))ww2[2]|=(af_val(f)<<b);
                            f=g2_res(&PA[i].sys,af_var(3*N+b));
                            if(af_is_const(f))ww2[3]|=(af_val(f)<<b);
                        }
                        uint32_t ws2;
