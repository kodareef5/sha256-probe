/*
 * kernel_sweep.c — Test all kernel bit positions at a given N
 * Usage: ./kernel_sweep N
 * Tests dM[0]=dM[9]=2^bit for bit=0..N-1, across fill patterns.
 * Reports collision count for each (kernel_bit, best_candidate).
 *
 * gcc -O3 -march=native -o kernel_sweep kernel_sweep.c -lm
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
static int SR(int k){int r=(int)rint((double)k*gN/32.0);return r<1?1:r;}
        ...
        for(uint32_t w58=0;w58<(1U<<gN);w58++){
            ...
            for(uint32_t w59=0;w59<(1U<<gN);w59++){
                ...
                for(uint32_t w60=0;w60<(1U<<gN);w60++){
                    ...
                    uint32_t w60b=find_w2(fa,fb,60,w60);sha_round(fa,KN[60],w60);sha_round(fb,KN[60],w60b);
                    uint32_t Wa[7]={w57,w58,w59,w60,0,0,0},Wb[7]={w57b,w58b,w59b,w60b,0,0,0};
                    Wa[4]=(fns1(Wa[2])+W1p[54]+fns0(W1p[46])+W1p[45])&gMASK;
                    Wb[4]=(fns1(Wb[2])+W2p[54]+fns0(W2p[46])+W2p[45])&gMASK;
                    Wa[5]=(fns1(Wa[3])+W1p[55]+fns0(W1p[47])+W1p[46])&gMASK;
                    Wb[5]=(fns1(Wb[3])+W2p[55]+fns0(W2p[47])+W2p[46])&gMASK;
                    Wa[6]=(fns1(Wa[4])+W1p[56]+fns0(W1p[48])+W1p[47])&gMASK;
                    Wb[6]=(fns1(Wb[4])+W2p[56]+fns0(W2p[48])+W2p[47])&gMASK;
...
int main(int argc, char *argv[]) {
    ...
    printf("Kernel sweep at N=%d\n", gN);
    printf("Testing dM[0]=dM[9]=2^bit for bit=0..%d\n\n", gN-1);
    printf("Bit  Fill    M[0]  Coll  Best?\n");

    int global_best = 0;
    for (int bit = 0; bit < gN; bit++) {
        uint32_t delta = 1U << bit;
        int best_nc = 0; uint32_t best_m0 = 0, best_fill = 0;
        ...
        for (int fi = 0; fi < 3; fi++) {
            for (uint32_t m0 = 0; m0 <= gMASK; m0++) {
                ...
                if (nc > best_nc) { best_nc = nc; best_m0 = m0; best_fill = fills[fi]; }
            }
        }
        char tag[8] = "";
        if (best_nc > global_best) { global_best = best_nc; strcpy(tag, " <<<"); }
        printf("  %d   0x%02x  0x%02x  %4d%s\n", bit, best_fill, best_m0, best_nc, tag);
    }
    printf("\nBest: %d collisions\n", global_best);
    return 0;
}
