...
#define MASK ((1U<<N)-1)
#define MSB (1U<<(N-1))
#define NV 16  /* W1[59..60] only, W2 from cascade */       /* 32 variables: W1[57..60] + W2[57..60] × N bits.
                        * Cascade constraints link them via GF(2). */
#define MAXPOOL 131072 /* 2M max states — need room for round 60 expansion */

/* ---- Affine Form: val = popcount(m & vars) % 2 XOR c ---- */
typedef struct { uint64_t m; uint8_t c; } AF;
...
static inline AF af_var(int i) { return (AF){1ULL<<i, 0}; }
...
int main(int argc, char *argv[]) {
    ...
    s1c[0]=0x45;s1c[1]=0xf8;s1c[2]=0x31;s1c[3]=0xae;s1c[4]=0x47;s1c[5]=0xdd;s1c[6]=0x6b;s1c[7]=0x9c;
    s2c[0]=0x45;s2c[1]=0x6a;s2c[2]=0x6f;s2c[3]=0x09;s2c[4]=0x8e;s2c[5]=0xa8;s2c[6]=0x00;s2c[7]=0x32;
    ...
    for (int i = 0; i < nA && i < 16; i++) {
        ...
        for (int r = 0; r < 8; r++) {
            ...
            for (int b = 0; b < N; b++) {
                ...
                if (!af_is_const(f)) {
                    printf("  FAIL: state %d reg %d bit %d still symbolic (mask=0x%llx)\n",
                           i, r, b, f.m);
                    test4_ok = 0; break;
                }
                sym_val |= (af_val(f) << b);
    ...
    printf("Phase 6: Outer W1[57]xW1[58], inner affine W1[59..60]\n");
    printf("  65536 outer pairs, 32 inner vars, max 65536 pool states\n\n");

    int total_collisions = 0;
    struct timespec ts0, ts1;
    clock_gettime(CLOCK_MONOTONIC, &ts0);

    uint32_t w57_start = argc>3 ? atoi(argv[3]) : 0; uint32_t w57_end = argc>4 ? atoi(argv[4]) : (1U<<N); for (uint32_t w57 = w57_start; w57 < w57_end; w57++) {
        uint32_t rest1=(s1c[7]+bfS1(s1c[4])+bfCh(s1c[4],s1c[5],s1c[6])+KN[57])&MASK;
        uint32_t rest2=(s2c[7]+bfS1(s2c[4])+bfCh(s2c[4],s2c[5],s2c[6])+KN[57])&MASK;
        uint32_t t21=(bfS0(s1c[0])+bfMj(s1c[0],s1c[1],s1c[2]))&MASK;
        uint32_t t22=(bfS0(s2c[0])+bfMj(s2c[0],s2c[1],s2c[2]))&MASK;
        uint32_t w2_57=(w57+rest1-rest2+t21-t22)&MASK;
        ...
        for (uint32_t w58 = 0; w58 < (1U << N); w58++) {
            uint32_t r1_58=(st57_1[7]+bfS1(st57_1[4])+bfCh(st57_1[4],st57_1[5],st57_1[6])+KN[58])&MASK;
            uint32_t r2_58=(st57_2[7]+bfS1(st57_2[4])+bfCh(st57_2[4],st57_2[5],st57_2[6])+KN[58])&MASK;
            uint32_t t21_58=(bfS0(st57_1[0])+bfMj(st57_1[0],st57_1[1],st57_1[2]))&MASK;
            uint32_t t22_58=(bfS0(st57_2[0])+bfMj(st57_2[0],st57_2[1],st57_2[2]))&MASK;
            uint32_t w2_58=(w58+r1_58-r2_58+t21_58-t22_58)&MASK;
            ...
            /* Inner affine engine: W1[59..60]+W2[59..60] = 32 vars */
            nA = 1; g2_init(&PA[0].sys);
            for(int r=0;r<8;r++) for(int b=0;b<N;b++) {
                PA[0].r1[r][b]=af_const((st58_1[r]>>b)&1);
                PA[0].r2[r][b]=af_const((st58_2[r]>>b)&1);
            }

            for(int rnd=2;rnd<7 && nA>0;rnd++) {
                int rr=57+rnd, is_free=(rnd<4);
                if(is_free) {
                    for(int i=0;i<nA;i++) for(int b=0;b<N;b++)
                        PA[i].w[b]=af_var((rnd-2)*N+b);
                } else {
                    for(int i=0;i<nA;i++) {
                        uint32_t ww[4]={w57,w58,0,0};
                        for(int b=0;b<N;b++){
                            AF f=g2_res(&PA[i].sys,af_var(b));
                            if(af_is_const(f))ww[2]|=(af_val(f)<<b);
                            f=g2_res(&PA[i].sys,af_var(N+b));
                            if(af_is_const(f))ww[3]|=(af_val(f)<<b);
                        }
                        uint32_t ws;
                        if(rr==61)ws=(bfs1(ww[2])+W1p[54]+bfs0(W1p[46])+W1p[45])&MASK;
                        else if(rr==62)ws=(bfs1(ww[3])+W1p[55]+bfs0(W1p[47])+W1p[46])&MASK;
                        else{uint32_t w61t=(bfs1(ww[2])+W1p[54]+bfs0(W1p[46])+W1p[45])&MASK;
                             ws=(bfs1(w61t)+W1p[56]+bfs0(W1p[48])+W1p[47])&MASK;}
                        for(int b=0;b<N;b++) PA[i].w[b]=af_const((ws>>b)&1);
                    }
                }
                nA=do_one_round(nA,rnd,0);
                if(nA==0)break;

                if(is_free) {
                    /* Cascade: W2[rnd] = W1[rnd] + offset(state_before_round) */
                    /* State before this round is concrete (from previous rounds) */
                    /* Extract concrete registers, compute cascade offset */
                    for(int i=0;i<nA;i++) {
                        uint32_t rs1[8]={0},rs2[8]={0};
                        for(int rr=0;rr<8;rr++) for(int bb=0;bb<N;bb++){
                            AF f1=g2_res(&PA[i].sys,PA[i].r1[rr][bb]);
                            AF f2=g2_res(&PA[i].sys,PA[i].r2[rr][bb]);
                            if(af_is_const(f1))rs1[rr]|=(af_val(f1)<<bb);
                            if(af_is_const(f2))rs2[rr]|=(af_val(f2)<<bb);
                        }
                        /* Cascade offset */
                        uint32_t rest1=(rs1[7]+bfS1(rs1[4])+bfCh(rs1[4],rs1[5],rs1[6])+KN[57+rnd])&MASK;
                        uint32_t rest2=(rs2[7]+bfS1(rs2[4])+bfCh(rs2[4],rs2[5],rs2[6])+KN[57+rnd])&MASK;
                        uint32_t tt21=(bfS0(rs1[0])+bfMj(rs1[0],rs1[1],rs1[2]))&MASK;
                        uint32_t tt22=(bfS0(rs2[0])+bfMj(rs2[0],rs2[1],rs2[2]))&MASK;
                        /* W1[rnd] value from GF2 system */
                        uint32_t w1v=0;
                        for(int bb=0;bb<N;bb++){
                            AF f=g2_res(&PA[i].sys,af_var((rnd-2)*N+bb));
                            if(af_is_const(f))w1v|=(af_val(f)<<bb);
                        }
                        uint32_t w2v=(w1v+rest1-rest2+tt21-tt22)&MASK;
                        for(int bb=0;bb<N;bb++)
                            PA[i].w[bb]=af_const((w2v>>bb)&1);
                    }
                } else {
                    for(int i=0;i<nA;i++) {
                        /* Recompute W2[59..60] from W1 via cascade */
                        uint32_t ww1_59=0,ww1_60=0;
                        for(int b=0;b<N;b++){
                            AF f=g2_res(&PA[i].sys,af_var(b));
                            if(af_is_const(f))ww1_59|=(af_val(f)<<b);
                            f=g2_res(&PA[i].sys,af_var(N+b));
                            if(af_is_const(f))ww1_60|=(af_val(f)<<b);
                        }
                        /* Need state before rounds 59,60 to compute cascade */
                        /* State before round 59 = st58 (concrete, from outer loop) */
                        /* But st58 was modified by rounds 59-60 in the affine engine */
                        /* PROBLEM: we don't have the pre-round-59 state anymore */
                        /* WORKAROUND: store the cascade W2 values during round processing */
                        /* For now: just use W1 schedule (WRONG for W2) */
                        /* TODO: fix this properly */
                        uint32_t ww2[4]={w2_57,w2_58,0,0};
                        ww2[2]=0;ww2[3]=0; /* placeholder — needs cascade W2 */
                        uint32_t ws2;
                        if(rr==61)ws2=(bfs1(ww2[2])+W2p[54]+bfs0(W2p[46])+W2p[45])&MASK;
                        else if(rr==62)ws2=(bfs1(ww2[3])+W2p[55]+bfs0(W2p[47])+W2p[46])&MASK;
                        else{uint32_t w61t=(bfs1(ww2[2])+W2p[54]+bfs0(W2p[46])+W2p[45])&MASK;
                             ws2=(bfs1(w61t)+W2p[56]+bfs0(W2p[48])+W2p[47])&MASK;}
                        for(int b=0;b<N;b++) PA[i].w[b]=af_const((ws2>>b)&1);
                    }
                }
                nA=do_one_round(nA,rnd,1);
                if(nA==0)break;

                /* Cascade W2 already computed — no separate constraint needed */
            }

            if(nA>0) {
                int nout=0;
                for(int i=0;i<nA;i++){
                    PB[nout]=PA[i];int ok=1;
                    for(int r=0;r<8&&ok;r++) for(int b=0;b<N&&ok;b++){
                        AF diff=af_xor(PA[i].r1[r][b],PA[i].r2[r][b]);
                        AF rd=g2_res(&PB[nout].sys,diff);
                        if(af_is_const(rd)){if(af_val(rd))ok=0;}
                        else{if(!g2_add(&PB[nout].sys,rd))ok=0;}
                    }
                    if(ok){nout++;total_collisions++;}
                }
        ...
        if((w57&0xF)==0xF){
            ...
            printf("  [%.0f%%] w57=0x%02x coll=%d %.1fs\n",
                   100.0*(w57+1-w57_start)/(w57_end-w57_start),w57,total_collisions,el);
        }
    }
    clock_gettime(CLOCK_MONOTONIC,&ts1);
    double elf=(ts1.tv_sec-ts0.tv_sec)+(ts1.tv_nsec-ts0.tv_nsec)/1e9;
    printf("\n=== N=8 Results ===\n");
    printf("Collisions: %d\n",total_collisions);
    printf("Time: %.1fs\n",elf);
    if(total_collisions==260) printf("\n*** ALL 260 COLLISIONS FOUND ***\n");
