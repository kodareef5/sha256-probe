/* Cascade-Optimized DP — Generic N with auto fill detection
 * Usage: cascade_dp_generic [N]
 * Compile: gcc -O3 -march=native -o cascade_dp_generic cascade_dp_generic.c -lm
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
int main(int argc,char*argv[]){
    ...
    /* Try multiple fills to find candidate */
    uint32_t fills[]={gMASK,0,gMASK>>1,gMSB,0x55&gMASK,0xAA&gMASK};
    int found=0; uint32_t use_fill=0;
    for(int fi=0;fi<6&&!found;fi++){
        for(uint32_t m0=0;m0<=gMASK&&!found;m0++){
            ...
            if(state1[0]==state2[0]){
                printf("Candidate: N=%d M[0]=0x%x fill=0x%x\n",gN,m0,fills[fi]);
                use_fill=fills[fi]; found=1;
            }
        }
    }
    if(!found){printf("No candidate at N=%d\n",gN);return 1;}

    uint64_t search=(uint64_t)1<<(4*gN);
    printf("Cascade DP: search 2^%d = %llu candidates\n\n",4*gN,(unsigned long long)search);

    struct timespec t0,t1;
    clock_gettime(CLOCK_MONOTONIC,&t0);
    uint64_t n_tested=0,n_coll=0;

    #pragma omp parallel for schedule(dynamic,1) reduction(+:n_coll,n_tested)
    for(uint32_t w57=0;w57<(1U<<gN);w57++){
        ...
        for(uint32_t w58=0;w58<(1U<<gN);w58++){
            ...
            for(uint32_t w59=0;w59<(1U<<gN);w59++){
                ...
                for(uint32_t w60=0;w60<(1U<<gN);w60++){
                    ...
                    if(ok){
                        n_coll++;
                        if(n_coll<=5)printf("  #%llu: W1=[%x,%x,%x,%x]\n",(unsigned long long)n_coll,w57,w58,w59,w60);
                    }
                }
            }
        }
        if((w57&0xF)==0xF&&gN>=8){
            ...
            printf("  [%.0f%%] w57=0x%x, %llu tested, %llu coll, %.1fs, ETA %.0fs\n",
                   pct,w57,(unsigned long long)n_tested,(unsigned long long)n_coll,el,
                   el/pct*100-el);
        }
    }
    clock_gettime(CLOCK_MONOTONIC,&t1);
    ...
    printf("\n=== N=%d Results ===\nTested: %llu\nCollisions: %llu\nTime: %.3fs\n",
           gN,(unsigned long long)n_tested,(unsigned long long)n_coll,elapsed);
    printf("Done.\n");
    return 0;
}
