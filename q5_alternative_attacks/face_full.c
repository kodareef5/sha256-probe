/*
 * face_full.c — FACE: Frontier Automaton via Carry Elimination (N=8)
 *
 * Symbolic solver using GF(2) affine forms with 3-mode addition branching.
 * 16 symbolic variables (W1[59..60] bits), outer loop W1[57] x W1[58].
 *
 * Architecture: pool-based processing for rounds 59-60 (symbolic), hybrid
 * concrete fallback for rounds 61-63 with de61=0 structural filter.
 * Cascade W2 = W1 + offset computed per-state via symbolic addition.
 *
 * Design note: attempts at all-32-variable symbolic processing showed that
 * cascade rounds multiply the pool by 256 with zero pruning benefit (the
 * cascade offset makes da=0 always satisfiable). The 2-symbolic-round
 * architecture is optimal for the pool-based approach.
 *
 * N=8, MSB kernel, M[0]=0x67, fill=0xff. Expected: 260 collisions.
 * Compile: gcc -O3 -march=native -o face_full face_full.c -lm
 */
...
#define MASK ((1U<<N)-1)
#define MSB (1U<<(N-1))
#define NV 16              /* Symbolic vars: W1[59] bits 0-7, W1[60] bits 8-15 */
#define MAXPOOL (1<<18)    /* 256K — matches face_n8.c */
#define M0 0x67U
...
static int p_maj(int n,int m2){
    for(int bit=0;bit<N;bit++){int no=0;
        for(int i=0;i<n&&no<MAXPOOL-4;i++){
            ...
            else{AF bxc=af_x(b,c);
                if(af_ic(bxc)){PB[no]=PA[i];PB[no].az[bit]=af_cv(bxc)?a:b;no++;}
                else{PB[no]=PA[i];
                    if(g2_add(&PB[no].sys,bxc)){stat_br++;PB[no].az[bit]=g2_r(&PB[no].sys,PA[i].ay[bit]);no++;}else stat_cx++;
                    if(no<MAXPOOL){PB[no]=PA[i];AF d=bxc;d.c^=1;
                    if(g2_add(&PB[no].sys,d)){stat_br++;PB[no].az[bit]=g2_r(&PB[no].sys,PA[i].ax[bit]);no++;}else stat_cx++;}
                }}}
        ST*t=PA;PA=PB;PB=t;n=no;}return n;}

static int do_rnd(int n,int abs_r,int m2){
    #define R(s,r) (m2?(s)->r2[r]:(s)->r1[r])
    for(int i=0;i<n;i++){memcpy(PA[i].ax,R(&PA[i],4),N*sizeof(AF));memcpy(PA[i].ay,R(&PA[i],5),N*sizeof(AF));}
    n=p_ch(n,m2);if(!n)return 0;for(int i=0;i<n;i++)memcpy(PA[i].ch,PA[i].az,N*sizeof(AF));
    for(int i=0;i<n;i++){memcpy(PA[i].ax,R(&PA[i],7),N*sizeof(AF));aS1(PA[i].ay,R(&PA[i],4));PA[i].ac=0;}
    n=p_add(n);if(!n)return 0;for(int i=0;i<n;i++)memcpy(PA[i].t1,PA[i].az,N*sizeof(AF));
    for(int i=0;i<n;i++){memcpy(PA[i].ax,PA[i].t1,N*sizeof(AF));memcpy(PA[i].ay,PA[i].ch,N*sizeof(AF));PA[i].ac=0;}
    n=p_add(n);if(!n)return 0;for(int i=0;i<n;i++)memcpy(PA[i].t1,PA[i].az,N*sizeof(AF));
    for(int i=0;i<n;i++){for(int b=0;b<N;b++)PA[i].ax[b]=af_c((KN[abs_r]>>b)&1);memcpy(PA[i].ay,PA[i].w,N*sizeof(AF));PA[i].ac=0;}
    n=p_add(n);if(!n)return 0;for(int i=0;i<n;i++)memcpy(PA[i].t2,PA[i].az,N*sizeof(AF));
    ...
    n=p_maj(n,m2);if(!n)return 0;for(int i=0;i<n;i++)memcpy(PA[i].maj,PA[i].az,N*sizeof(AF));
    ...
    n=p_add(n);if(!n)return 0;for(int i=0;i<n;i++)memcpy(PA[i].ch,PA[i].az,N*sizeof(AF));
    for(int i=0;i<n;i++){memcpy(PA[i].ax,PA[i].t1,N*sizeof(AF));memcpy(PA[i].ay,PA[i].t2,N*sizeof(AF));PA[i].ac=0;}
...
static int prune_all(int n){
    int no=0;
    for(int i=0;i<n&&no<MAXPOOL;i++){PB[no]=PA[i];int ok=1;
        for(int r=0;r<8&&ok;r++)for(int b=0;b<N&&ok;b++){
            AF diff=af_x(PA[i].r1[r][b],PA[i].r2[r][b]);AF rd=g2_r(&PB[no].sys,diff);
            if(af_ic(rd)){if(af_cv(rd))ok=0;}else{if(!g2_add(&PB[no].sys,rd))ok=0;}}
        if(ok)no++;}
    ST*t=PA;PA=PB;PB=t;return no;}

static int bf_collisions(uint32_t(*sols)[4],int max_sols){
    int count=0;
    for(uint64_t x=0;x<(1ULL<<(4*N));x++){
        ...
        uint32_t W1f[7],W2f[7],w2[4];
        W1f[0]=w1[0];W1f[1]=w1[1];W1f[2]=w1[2];W1f[3]=w1[3];
        W1f[4]=(fn_s1(w1[2])+W1p[54]+fn_s0(W1p[46])+W1p[45])&MASK;
        W1f[5]=(fn_s1(w1[3])+W1p[55]+fn_s0(W1p[47])+W1p[46])&MASK;
        W1f[6]=(fn_s1(W1f[4])+W1p[56]+fn_s0(W1p[48])+W1p[47])&MASK;
        for(int r=0;r<4;r++){uint32_t off=cas_off(sa,sb,57+r);w2[r]=(W1f[r]+off)&MASK;
            bf_round(sa,KN[57+r],W1f[r]);bf_round(sb,KN[57+r],w2[r]);}
        W2f[0]=w2[0];W2f[1]=w2[1];W2f[2]=w2[2];W2f[3]=w2[3];
        W2f[4]=(fn_s1(w2[2])+W2p[54]+fn_s0(W2p[46])+W2p[45])&MASK;
        W2f[5]=(fn_s1(w2[3])+W2p[55]+fn_s0(W2p[47])+W2p[46])&MASK;
        W2f[6]=(fn_s1(W2f[4])+W2p[56]+fn_s0(W2p[48])+W2p[47])&MASK;
        for(int r=4;r<7;r++){bf_round(sa,KN[57+r],W1f[r]);bf_round(sb,KN[57+r],W2f[r]);}
        int ok=1;for(int r=0;r<8;r++)if(sa[r]!=sb[r]){ok=0;break;}
        if(ok){if(count<max_sols)memcpy(sols[count],w1,16);count++;}}
    return count;}
...
int main(int argc,char**argv){
    ...
    printf("=== FACE Full Symbolic Solver N=%d ===\n",N);
    printf("16 vars (W1[59..60]), outer W1[57]xW1[58] (256x256)\n");
    printf("3-mode adds, GF(2) RREF, de61=0 filter, hybrid r61-63\n\n");
    ...
    if(!skip_bf){
        ...
        bf_count=bf_collisions(bf_sols,512);
    ...
    for(uint32_t w57=0;w57<(1U<<N);w57++){
        ...
        for(uint32_t w58=0;w58<(1U<<N);w58++){
            ...
            sn=do_rnd(sn,59,0);if(!sn)goto next;

            /* R59 msg2: W2[59] = W1[59]+cas59 */
            ...
            sn=do_rnd(sn,59,1);if(!sn)goto next;

            /* Hybrid: extract concrete W59, iterate W60 */
            for(int si=0;si<sn;si++){
                ...
                uint32_t W2_61=(fn_s1(w2_59)+W2p[54]+fn_s0(W2p[46])+W2p[45])&MASK;
                ...
                for(uint32_t w60=0;w60<(1U<<N);w60++){
                    ...
                    if(((s61_1[4]-s61_2[4])&MASK)!=0)continue; /* de61=0 filter */
    ...
    printf("\n========================================\n");
    printf("     FACE Full Symbolic Results (N=%d)\n",N);
    printf("========================================\n");
