...
def hillclimb(short, m0, fill, kbit, iterations, seed=0, verbose=False):
    ...
    for i in range(iterations):
        n_flips = rng.choice([1, 1, 1, 2, 2, 3])
        new_W = list(cur_W)
        ...
        if sc["score"] is None:
            bridge_rejects += 1
            continue
        # Greedy: accept if strictly better
        if sc["score"] > cur_score:
            cur_W = tuple(new_W)
            cur_rec = rec
            cur_score = sc["score"]
            accepts += 1
            if cur_score > best_score:
                ...
                best_rec = cur_rec
            if verbose:
                print(f"    iter {i:5d}: new best score={best_score:.2f} hw={rec['hw_total']:3d}")

    return {
        "cand": short, "m0": f"0x{m0:08x}", "fill": f"0x{fill:08x}", "kernel_bit": kbit,
        "iterations": iterations, "seed": seed,
        "accepts": accepts, "cascade_rejects": cascade_rejects, "bridge_rejects": bridge_rejects,
...
def main():
    ...
    print(f"=== block2_bridge_beam.py: hillclimb {args.iterations} iters × {args.seeds} seeds × {len(CANDS)} cands ===\n")
    t0 = time.time()
    all_runs = []
    for short, m0, fill, kbit in CANDS:
        for seed in range(args.seeds):
            ...
            if "error" in r:
                ...
                print(f"    best score={r['best_score']:.2f}  hw={rec['hw_total']:3d}  "
                      f"accepts={r['accepts']:5d}  cascade_rej={r['cascade_rejects']:5d}  "
    ...
    # Save
    out_path = args.out or "headline_hunt/bets/block2_wang/results/search_artifacts/20260430_F379_bridge_beam.json"
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    with open(out_path, "w") as f:
        json.dump({
            "description": "F379: bridge-guided W-space hillclimb on bit3/bit2/bit28 + bit13_m916a56aa control",
