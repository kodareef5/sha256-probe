#!/usr/bin/env python3
"""n10_invariants.py — Check Theorem 4 + R63.x modular relations at N=10.

The cascade structure derived at N=32 predicts:
  Theorem 4 (at r=61):     da_61 ≡ de_61 (mod 2^N)
  R63.1:                   dc_63 ≡ dg_63 (mod 2^N)
  R63.3:                   da_63 - de_63 ≡ dT2_63 (mod 2^N)

These are the relations from bets/mitm_residue/results/20260425_residual_structure_complete.md.
Test if they hold at N=10 too — sample W57 random, force cascade,
extend through schedule, check the relations across many samples.

Usage: python3 n10_invariants.py
"""
import random
...
rs0 = (scale_rot(7),  scale_rot(18), scale_rot(3))   # last is shift, not rot
...
def sigma0(x):
    return ror(x, rs0[0]) ^ ror(x, rs0[1]) ^ ((x >> rs0[2]) & MASK)


def sigma1(x):
    return ror(x, rs1[0]) ^ ror(x, rs1[1]) ^ ((x >> rs1[2]) & MASK)
...
# K, IV constants from SHA-256, masked to N bits
K32 = [
...
def apply_round(state, w, r):
    ...
    return (add(T1, T2), state[0], state[1], state[2],
            add(state[3], T1), state[4], state[5], state[6])
...
def main():
    ...
    print(f"N={N}, MASK=0x{MASK:x}, MSB-kernel auto-discovered M0=0x{m0:x}")
    fill = MASK
    M1 = [m0] + [fill] * 15
    M2 = list(M1)
    M2[0] ^= 1 << (N - 1)
    M2[9] ^= 1 << (N - 1)
    s1, W1pre = precompute_state(M1)
    s2, W2pre = precompute_state(M2)
    cw57 = cascade1_offset(s1, s2)

    rng = random.Random(42)
    n_samples = 1 << 14   # 16384

    # Theorem 4: da_61 ≡ de_61 (mod 2^N)
    th4_violations = 0
    # R63.1: dc_63 ≡ dg_63
    r63_1_violations = 0
    # R63.3: da_63 - de_63 ≡ dT2_63
    r63_3_violations = 0
    ...
    for _ in range(n_samples):
        ...
        cw59 = cascade1_offset(s1_58, s2_58)
        s1_59 = apply_round(s1_58, 0, 59)
        s2_59 = apply_round(s2_58, cw59, 59)
        cw60 = cascade2_offset(s1_59, s2_59)
        s1_60 = apply_round(s1_59, 0, 60)
        s2_60 = apply_round(s2_59, cw60, 60)
        # schedule W[61..63]
        W1 = list(W1pre) + [w57, 0, 0, 0]
        W2 = list(W2pre) + [w2_57, cw58, cw59, cw60]
        for r in (61, 62, 63):
        ...
        # dT2_63 = dSigma0(a_62) + dMaj(a_62, b_62, c_62)
        dSig0 = (Sigma0(s1_62[0]) - Sigma0(s2_62[0])) & MASK
        dMaj = (Maj(s1_62[0], s1_62[1], s1_62[2])
                - Maj(s2_62[0], s2_62[1], s2_62[2])) & MASK
        dT2_63 = (dSig0 + dMaj) & MASK
        if (da63 - de63) & MASK != dT2_63:
            r63_3_violations += 1

    print(f"\nN={N} invariant checks across {n_samples} cascade-1+2 samples:")
    print(f"  Theorem 4 (da_61 = de_61 mod 2^N):         {n_samples - th4_violations}/{n_samples}")
    print(f"  R63.1     (dc_63 = dg_63):                  {n_samples - r63_1_violations}/{n_samples}")
    print(f"  R63.3     (da_63 - de_63 = dT2_63 mod 2^N): {n_samples - r63_3_violations}/{n_samples}")
    if th4_violations == 0 and r63_1_violations == 0 and r63_3_violations == 0:
        print("\nAll three modular relations hold at N=10 — EVIDENCE that the structural")
        print("picture from N=32 generalizes downward. Predicts upward generalization.")
    else:
        print("\nVIOLATIONS DETECTED — structural picture does NOT generalize cleanly to N=10.")
        print("Investigate which relation breaks; the cascade construction may behave")
        print("differently at small N due to scaled rotation parameters.")
