/*
 * prefix_sweep.c — Sweep random (W1[57], W1[58], W1[59]) prefixes
 *                  and count how many produce collisions.
 *
 * For each prefix:
 * 1. Run cascade chain to round 59
 * 2. Enumerate all 2^32 W1[60] values
 * 3. Count those that pass round 61, 62, 63 cascade closures
 * 4. Among those, count actual collisions
 *
 * Question: how rare is a "collision-prefix"?
 *
 * Compile: gcc -O3 -march=native -fopenmp -o /tmp/psweep q4_mitm_geometry/prefix_sweep.c
 */
...
int main(int argc, char **argv) {
    long n_prefixes = argc > 1 ? atol(argv[1]) : 100;
    ...
    uint32_t sched_const1_62 = W1_pre[55] + sigma0(W1_pre[47]) + W1_pre[46];
    uint32_t sched_const2_62 = W2_pre[55] + sigma0(W2_pre[47]) + W2_pre[46];
    uint32_t sched_const1_63 = W1_pre[56] + sigma0(W1_pre[48]) + W1_pre[47];
    uint32_t sched_const2_63 = W2_pre[56] + sigma0(W2_pre[48]) + W2_pre[47];

    fprintf(stderr, "Sweeping %ld random prefixes...\n", n_prefixes);
    fprintf(stderr, "Each does full 2^32 W1[60] enumeration with rounds 61,62,63 closure.\n\n");

    uint64_t rng_state = (uint64_t)time(NULL) | 1;

    long total_61 = 0, total_62 = 0, total_63 = 0, total_collisions = 0;
    long prefixes_with_61 = 0, prefixes_with_62 = 0, prefixes_with_63 = 0, prefixes_with_collision = 0;
    ...
    for (long p = 0; p < n_prefixes; p++) {
        ...
        /* Schedule W[61] target depends on (w1_59, w2_59) */
        uint32_t sched_dW61 = sigma1(w2_59) - sigma1(w1_59) + sched_const2_61 - sched_const1_61;

        long m61 = 0, m62 = 0, m63 = 0, coll = 0;

        #pragma omp parallel for reduction(+:m61,m62,m63,coll)
        for (long w1_60_l = 0; w1_60_l < (1L << 32); w1_60_l++) {
            ...
            if (rc61 != sched_dW61) continue;
            m61++;

            uint32_t w1_61 = sigma1(w1_59) + sched_const1_61;
            uint32_t w2_61 = sigma1(w2_59) + sched_const2_61;
            uint32_t s1e[8], s2e[8];
            one_round(s1e, s1d, w1_61, 61);
            one_round(s2e, s2d, w2_61, 61);
            uint32_t rc62 = cascade_dw(s1e, s2e);
            uint32_t sched_dW62 = sigma1(w2_60) - sigma1(w1_60) + sched_const2_62 - sched_const1_62;
            if (rc62 != sched_dW62) continue;
            m62++;

            uint32_t w1_62 = sigma1(w1_60) + sched_const1_62;
            uint32_t w2_62 = sigma1(w2_60) + sched_const2_62;
            uint32_t s1f[8], s2f[8];
            one_round(s1f, s1e, w1_62, 62);
            one_round(s2f, s2e, w2_62, 62);
            uint32_t rc63 = cascade_dw(s1f, s2f);
            uint32_t sched_dW63 = sigma1(w2_61) - sigma1(w1_61) + sched_const2_63 - sched_const1_63;
            if (rc63 != sched_dW63) continue;
            m63++;

            uint32_t w1_63 = sigma1(w1_61) + sched_const1_63;
            uint32_t w2_63 = sigma1(w2_61) + sched_const2_63;
            uint32_t s1g[8], s2g[8];
            one_round(s1g, s1f, w1_63, 63);
            one_round(s2g, s2f, w2_63, 63);
            int total = 0;
            for (int r = 0; r < 8; r++) total += hw(s1g[r] ^ s2g[r]);
            if (total == 0) {
                coll++;
                #pragma omp critical
                {
                    fprintf(stderr, "*** COLLISION at prefix %ld: W1[57..60] = 0x%08x 0x%08x 0x%08x 0x%08x\n",
                            p, w1_57, w1_58, w1_59, w1_60);
                }
            }
        }

        total_61 += m61; total_62 += m62; total_63 += m63; total_collisions += coll;
        if (m61 > 0) prefixes_with_61++;
        if (m62 > 0) prefixes_with_62++;
        if (m63 > 0) prefixes_with_63++;
        if (coll > 0) prefixes_with_collision++;

        if (p < 5 || p % 10 == 0) {
            time_t now = time(NULL);
            fprintf(stderr, "[p=%ld, %lds] m61=%ld m62=%ld m63=%ld coll=%ld | totals: 61=%ld 62=%ld 63=%ld coll=%ld\n",
                    p, (long)(now-t0), m61, m62, m63, coll,
                    total_61, total_62, total_63, total_collisions);
        }
    }

    time_t t1 = time(NULL);
    fprintf(stderr, "\nDone in %lds\n", (long)(t1-t0));
    fprintf(stderr, "Prefixes tested: %ld\n", n_prefixes);
    fprintf(stderr, "Prefixes with round 61 match: %ld\n", prefixes_with_61);
    fprintf(stderr, "Prefixes with round 62 match: %ld\n", prefixes_with_62);
    fprintf(stderr, "Prefixes with round 63 match: %ld\n", prefixes_with_63);
    fprintf(stderr, "Prefixes with full collision: %ld\n", prefixes_with_collision);
    fprintf(stderr, "Total round 61 matches: %ld\n", total_61);
    fprintf(stderr, "Total round 62 matches: %ld\n", total_62);
    fprintf(stderr, "Total round 63 matches: %ld\n", total_63);
    fprintf(stderr, "Total collisions: %ld\n", total_collisions);
