/* Compute the image of cascade_dw at round 60 over all 2^32 W1[60] values
 * for the cert prefix. Then check what fraction of all 2^32 possible target
...
int main() {
    ...
    uint32_t target = sigma1(cert_w1_59 + C59) - sigma1(cert_w1_59) + sched_const2_61 - sched_const1_61;
    fprintf(stderr, "Cert target dW61 = 0x%08x\n", target);

    /* Build a histogram of cascade_dw values over all 2^32 W1[60].
     * Use a hash set to count distinct values. With 2^32 inputs and potentially
     * 2^32 distinct outputs, naive set is impractical. Use bitarray of 2^32 bits
     * (512 MB) on the server. */

    /* Allocate 2^32 bits = 512 MB */
    uint8_t *seen = calloc((1L << 32) / 8, 1);
    if (!seen) { fprintf(stderr, "alloc failed\n"); return 1; }

    long n_evals = 0;
    long n_target_hits = 0;

    fprintf(stderr, "Computing image of cascade_dw over 2^32 W1[60] values...\n");
    time_t t0 = time(NULL);

    #pragma omp parallel for reduction(+:n_evals,n_target_hits)
    ...
    fprintf(stderr, "\nDone in %lds\n", (long)(t1-t0));
    fprintf(stderr, "Total evaluations: %ld\n", n_evals);
    fprintf(stderr, "Distinct cascade_dw values: %ld\n", distinct);
    fprintf(stderr, "Image fraction: %.4f%% of 2^32\n", 100.0 * distinct / (double)(1L << 32));
    fprintf(stderr, "Target hits (cert): %ld (should be 8192)\n", n_target_hits);
