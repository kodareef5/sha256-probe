/*
 * cascade_chain_kernel.c — Cascade chain search with configurable kernel.
 *
 * Like cascade_chain_search.c but allows arbitrary kernel diff (not just MSB).
 * Tests gpu-laptop's new best candidate: M[0]=0x67dd2607, kernel=0x00000004 (bit 2).
 *
 * Compile: gcc -O3 -march=native -fopenmp -o /tmp/casc_kern q4_mitm_geometry/cascade_chain_kernel.c
 * Run: /tmp/casc_kern [n_samples] [m0] [fill] [kernel_diff]
 */
...
int main(int argc, char **argv) {
    long n_samples = argc > 1 ? atol(argv[1]) : 100000000L;
    uint32_t m0 = argc > 2 ? (uint32_t)strtoul(argv[2], NULL, 16) : 0x17149975;
    uint32_t fill = argc > 3 ? (uint32_t)strtoul(argv[3], NULL, 16) : 0xffffffff;
    uint32_t kernel = argc > 4 ? (uint32_t)strtoul(argv[4], NULL, 16) : 0x80000000;

    fprintf(stderr, "Cascade chain search (custom kernel)\n");
    fprintf(stderr, "M[0]=0x%08x, fill=0x%08x, kernel=0x%08x\n", m0, fill, kernel);
    ...
    if (s1_init[0] != s2_init[0]) {
        fprintf(stderr, "ERROR: da[56] != 0 for this candidate\n");
        return 1;
    }

    int total_state_hw = 0;
    for (int r = 0; r < 8; r++) total_state_hw += hw(s1_init[r] ^ s2_init[r]);
    fprintf(stderr, "State HW at round 56: %d\n", total_state_hw);

    uint32_t C57 = cascade_dw(s1_init, s2_init);
    fprintf(stderr, "C57 = 0x%08x\n\n", C57);

    int best_hw = 256;
    uint32_t best_w1_57 = 0, best_w1_58 = 0, best_w1_59 = 0, best_w1_60 = 0;
    ...
    #pragma omp parallel
    {
        uint64_t rng_state = (uint64_t)time(NULL) ^ ((uint64_t)omp_get_thread_num() << 32);
        rng_state |= 1;

        int local_best = 256;
        uint32_t lb[4] = {0,0,0,0};

        #pragma omp for reduction(+:total_evaluated)
        for (long iter = 0; iter < n_samples; iter++) {
            total_evaluated++;
            uint32_t w1_57 = (uint32_t)(xorshift(&rng_state) & MASK);
            ...
            uint32_t w1_58 = (uint32_t)(xorshift(&rng_state) & MASK);
            ...
            uint32_t w1_59 = (uint32_t)(xorshift(&rng_state) & MASK);
            ...
            uint32_t w1_60 = (uint32_t)(xorshift(&rng_state) & MASK);
            uint32_t w2_60 = w1_60 + C60;
            uint32_t s1d[8], s2d[8];
            one_round(s1d, s1c, w1_60, 60);
            one_round(s2d, s2c, w2_60, 60);

            uint32_t w1_61 = sigma1(w1_59) + W1_pre[54] + sigma0(W1_pre[46]) + W1_pre[45];
            uint32_t w2_61 = sigma1(w2_59) + W2_pre[54] + sigma0(W2_pre[46]) + W2_pre[45];
            uint32_t s1e[8], s2e[8];
            one_round(s1e, s1d, w1_61, 61);
            one_round(s2e, s2d, w2_61, 61);

            uint32_t w1_62 = sigma1(w1_60) + W1_pre[55] + sigma0(W1_pre[47]) + W1_pre[46];
            uint32_t w2_62 = sigma1(w2_60) + W2_pre[55] + sigma0(W2_pre[47]) + W2_pre[46];
            uint32_t s1f[8], s2f[8];
            one_round(s1f, s1e, w1_62, 62);
            one_round(s2f, s2e, w2_62, 62);

            uint32_t w1_63 = sigma1(w1_61) + W1_pre[56] + sigma0(W1_pre[48]) + W1_pre[47];
            uint32_t w2_63 = sigma1(w2_61) + W2_pre[56] + sigma0(W2_pre[48]) + W2_pre[47];
            ...
            if (total < local_best) {
                local_best = total;
                lb[0] = w1_57; lb[1] = w1_58; lb[2] = w1_59; lb[3] = w1_60;
            }
        }

        #pragma omp critical
        {
            if (local_best < best_hw) {
                best_hw = local_best;
                best_w1_57 = lb[0]; best_w1_58 = lb[1];
                best_w1_59 = lb[2]; best_w1_60 = lb[3];
            }
        }
    }
    ...
    fprintf(stderr, "Total samples: %ld (%.0f/s)\n", total_evaluated, (double)total_evaluated / elapsed);
    fprintf(stderr, "Best HW: %d\n", best_hw);
    fprintf(stderr, "Best W1[57..60]: 0x%08x 0x%08x 0x%08x 0x%08x\n",
            best_w1_57, best_w1_58, best_w1_59, best_w1_60);

    if (best_hw == 0) {
        fprintf(stderr, "\n*** COLLISION FOUND ***\n");
    }

    printf("M0=0x%08x KERNEL=0x%08x BEST_HW=%d SAMPLES=%ld\n",
           m0, kernel, best_hw, total_evaluated);
