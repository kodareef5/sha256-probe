/*
 * round62_filter.c — Of the round-61 matches, how many also satisfy round 62?
 *
 * For the cert prefix, enumerate W1[60] over 2^32, check both round-61 AND
 * round-62 closure conditions. Count survivors.
 *
 * Round 62 cascade closure:
 *   The schedule W[62] = sigma1(W[60]) + W[55] + sigma0(W[47]) + W[46]
 *   Required dW[62] = cascade_dw(state61) for da62=0
 *
 * State at round 61 depends on W[61] (schedule-derived from W[59]) and
 * the state at round 60 which depends on W[60].
 *
 * Compile: gcc -O3 -march=native -fopenmp -o /tmp/r62fil q4_mitm_geometry/round62_filter.c
 */
...
int main() {
    ...
    one_round(s1a, s1, 0x9ccfa55e, 57);
    one_round(s2a, s2, 0x9ccfa55e + C57, 57);

    uint32_t C58 = cascade_dw(s1a, s2a);
    uint32_t s1b[8], s2b[8];
    one_round(s1b, s1a, 0xd9d64416, 58);
    one_round(s2b, s2a, 0xd9d64416 + C58, 58);
    ...
    /* W[61] depends on W1[59] (cert), so target for round 61 is fixed */
    uint32_t cert_w2_59 = cert_w1_59 + C59;
    uint32_t sched_dW61 = sigma1(cert_w2_59) - sigma1(cert_w1_59) + sched_const2_61 - sched_const1_61;

    fprintf(stderr, "Searching all 2^32 W1[60] for cascades through rounds 61, 62, 63...\n");
    fprintf(stderr, "Target_dW61: 0x%08x\n", sched_dW61);
    fprintf(stderr, "C60 = 0x%08x\n\n", C60);

    long matches_61 = 0, matches_62 = 0, matches_63 = 0, full_collision = 0;
    int best_total_hw = 256;
    uint32_t best_w1_60 = 0;

    time_t t0 = time(NULL);

    #pragma omp parallel for reduction(+:matches_61,matches_62,matches_63,full_collision)
    for (long w1_60_l = 0; w1_60_l < (1L << 32); w1_60_l++) {
        ...
        /* Round 61 OK. Now check round 62. */
        uint32_t w1_61 = sigma1(cert_w1_59) + sched_const1_61;
        uint32_t w2_61 = sigma1(cert_w2_59) + sched_const2_61;
        ...
        /* Round 62 OK. Now check round 63. */
        ...
        /* Round 63 OK. Run final round and check collision. */
        ...
        if (total == 0) {
            ...
            #pragma omp critical
            {
                fprintf(stderr, "*** COLLISION FOUND: w1_60 = 0x%08x ***\n", w1_60);
            }
        }

        if (total < best_total_hw) {
            #pragma omp critical
            {
                if (total < best_total_hw) {
                    best_total_hw = total;
    ...
    time_t t1 = time(NULL);
    fprintf(stderr, "\nDone in %lds\n", (long)(t1-t0));
    fprintf(stderr, "Cert prefix: W1[57..59] = 0x9ccfa55e, 0xd9d64416, 0x9e3ffb08\n");
    fprintf(stderr, "Round 61 matches: %ld\n", matches_61);
    fprintf(stderr, "  + Round 62 matches: %ld\n", matches_62);
    fprintf(stderr, "  + Round 63 matches: %ld\n", matches_63);
    fprintf(stderr, "  Full collisions: %ld\n", full_collision);
    fprintf(stderr, "Best HW (best W1[60] = 0x%08x): %d\n", best_w1_60, best_total_hw);

    printf("M61 %ld M62 %ld M63 %ld COLL %ld BEST_HW %d\n",
           matches_61, matches_62, matches_63, full_collision, best_total_hw);
