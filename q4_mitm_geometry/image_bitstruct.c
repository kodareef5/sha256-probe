/* Analyze the bit-level structure of cascade_dw image for cert prefix.
 * Compute AND, OR, and bit popcounts across all values in image. */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
int main() {
    ...
    /* Compute image bit statistics */
    uint32_t and_mask = MASK, or_mask = 0;
    long bit_counts[32] = {0};
    long n_distinct = 0;
    ...
    fprintf(stderr, "Computing cert image bit structure...\n");
    time_t t0 = time(NULL);

    /* First pass: mark all values in image */
    ...
    /* Second pass: iterate through bitmap and accumulate stats */
    fprintf(stderr, "Image computed in %lds, analyzing...\n", (long)(time(NULL)-t0));
    ...
    fprintf(stderr, "\nImage size: %ld\n", n_distinct);
    fprintf(stderr, "AND of all values:  0x%08x  (bits always 1: %d)\n", and_mask, hw(and_mask));
    fprintf(stderr, "OR of all values:   0x%08x  (bits sometimes 1)\n", or_mask);
    fprintf(stderr, "Always 0 bits:      0x%08x  (count: %d)\n", ~or_mask & MASK, hw(~or_mask & MASK));
    fprintf(stderr, "Variable bits:      %d\n\n", 32 - hw(and_mask) - hw(~or_mask & MASK));

    fprintf(stderr, "Per-bit popularity (fraction set to 1):\n");
    for (int b = 0; b < 32; b++) {
        double frac = (double)bit_counts[b] / n_distinct;
        fprintf(stderr, "  bit %2d: %.4f", b, frac);
        if (frac < 0.01 || frac > 0.99) fprintf(stderr, "  ***");
        else if (frac < 0.1 || frac > 0.9) fprintf(stderr, "  *");
        fprintf(stderr, "\n");
    }
