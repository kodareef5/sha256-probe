/* Same as w59fam but with all 8 free bits including 17 and 31 */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
int main() {
    ...
    /* All 8 free bits including 17 and 31 */
    int free_bits[] = {11, 12, 16, 17, 28, 29, 31};
    int n_free = 7;
    int n_combos = 1 << n_free;  /* 128 */

    fprintf(stderr, "Testing 128 combinations of free bits {11,12,16,17,28,29,31}:\n\n");
    long total_pass = 0;
    long total_full = 0;
    long total_half = 0;
    ...
    for (int combo = 0; combo < n_combos; combo++) {
        ...
        for (int i = 0; i < n_free; i++) {
            if ((combo >> i) & 1) w59 ^= (1U << free_bits[i]);
        }
        long m = count_r61(cert_w1_57, cert_w1_58, w59, s1, s2, C57, W1_pre, W2_pre);
        if (m == 8192) total_full++;
        if (m == 4096) total_half++;
        if (m > 0) total_pass++;
        if (combo < 5 || (combo & 0x1f) == 0)
            fprintf(stderr, "  combo 0x%02x: w59=0x%08x, matches=%ld\n", combo, w59, m);
    }
    fprintf(stderr, "\nTotal pass: %ld of %d\n", total_pass, n_combos);
    fprintf(stderr, "  full (8192): %ld\n", total_full);
    fprintf(stderr, "  half (4096): %ld\n", total_half);
    fprintf(stderr, "Total round-61 matches across family: %ld\n",
            total_full * 8192 + total_half * 4096);
    return 0;
}
