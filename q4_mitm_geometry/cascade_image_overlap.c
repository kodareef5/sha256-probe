/* Compare cascade_dw images for multiple prefixes.
 * Compute image for prefix A, then for prefix B, count overlap. */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
/* Compute image of cascade_dw for given prefix into bitmap.
 * Returns count of distinct values. */
static long compute_image(uint32_t w1_57, uint32_t w1_58, uint32_t w1_59,
                           const uint32_t s1[8], const uint32_t s2[8],
                           uint32_t C57, uint8_t *seen) {
...
int main() {
    ...
    /* Allocate two 512MB bitmaps */
    uint8_t *seen_A = calloc((1L << 32) / 8, 1);
    uint8_t *seen_B = calloc((1L << 32) / 8, 1);
    if (!seen_A || !seen_B) { fprintf(stderr, "alloc failed\n"); return 1; }

    /* Prefix A: cert */
    uint32_t cert_w1_57 = 0x9ccfa55e, cert_w1_58 = 0xd9d64416, cert_w1_59 = 0x9e3ffb08;
    fprintf(stderr, "Computing image for CERT prefix...\n");
    time_t t0 = time(NULL);
    long size_A = compute_image(cert_w1_57, cert_w1_58, cert_w1_59, s1, s2, C57, seen_A);
    fprintf(stderr, "Cert image: %ld distinct values (%lds)\n\n", size_A, (long)(time(NULL)-t0));

    /* Prefix B: random */
    uint64_t rng = 0x123456789abcULL;
    uint32_t w1_57 = (uint32_t)xs(&rng);
    uint32_t w1_58 = (uint32_t)xs(&rng);
    uint32_t w1_59 = (uint32_t)xs(&rng);
    fprintf(stderr, "Computing image for RANDOM prefix (W1[57..59]=0x%08x 0x%08x 0x%08x)...\n",
            w1_57, w1_58, w1_59);
    t0 = time(NULL);
    long size_B = compute_image(w1_57, w1_58, w1_59, s1, s2, C57, seen_B);
    fprintf(stderr, "Random image: %ld distinct values (%lds)\n\n", size_B, (long)(time(NULL)-t0));

    /* Compute overlap */
    long overlap = 0;
    long union_count = 0;
    #pragma omp parallel for reduction(+:overlap,union_count)
    for (long i = 0; i < (1L << 32) / 8; i++) {
        overlap += __builtin_popcount(seen_A[i] & seen_B[i]);
        union_count += __builtin_popcount(seen_A[i] | seen_B[i]);
    }

    fprintf(stderr, "Overlap (intersection): %ld\n", overlap);
    fprintf(stderr, "Union: %ld\n", union_count);
    fprintf(stderr, "Cert image fraction in union: %.2f%%\n", 100.0 * size_A / union_count);
    fprintf(stderr, "Random image fraction in union: %.2f%%\n", 100.0 * size_B / union_count);
    fprintf(stderr, "Jaccard similarity: %.4f\n", (double)overlap / union_count);

    /* Expected if independent: A∩B = A*B / 2^32 = size_A * size_B / 2^32 */
    double expected_indep = (double)size_A * (double)size_B / (double)(1L << 32);
    fprintf(stderr, "\nExpected overlap if independent: %.0f\n", expected_indep);
    fprintf(stderr, "Observed / expected: %.2f\n", overlap / expected_indep);
    if (overlap > 2 * expected_indep) {
        fprintf(stderr, "*** SIGNIFICANT OVERLAP — images are correlated ***\n");
    } else if (overlap < 0.5 * expected_indep) {
        fprintf(stderr, "*** SIGNIFICANT ANTI-CORRELATION ***\n");
    } else {
        fprintf(stderr, "Images appear independent.\n");
    }
