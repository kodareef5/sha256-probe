/*
 * prefix_features.c — Compute structural features of cert and failed prefixes.
 *
 * For cert + many random prefixes, compute:
 * - State at round 60 (additive diffs of all 8 registers)
 * - dCh contribution at round 60 (constant part - varying part)
 * - Sigma1 contributions from various intermediate states
 * - W[59] derived schedule_dW61 target value
 *
 * Compare cert features vs failed prefixes to find the discriminator.
 *
 * Compile: gcc -O3 -march=native -fopenmp -o /tmp/pfeat q4_mitm_geometry/prefix_features.c
 */
...
/* Quick check: does ANY W1[60] in [0, max_test) pass round-61 closure? */
static int prefix_passes_round61(const uint32_t s1c[8], const uint32_t s2c[8],
                                  uint32_t C60, uint32_t target_dW61, long max_test) {
    int found = 0;
    #pragma omp parallel for shared(found)
    for (long w1_60_l = 0; w1_60_l < max_test; w1_60_l++) {
        if (found) continue;
...
int main(int argc, char **argv) {
    long n_prefixes = argc > 1 ? atol(argv[1]) : 1000;
    ...
    /* For each prefix, compute features and test round-61 quickly */
    fprintf(stderr, "Computing features for %ld prefixes...\n", n_prefixes);
    fprintf(stderr, "Quick test: 2^28 W1[60] per prefix (full enum if pass detected)\n\n");

    /* Cert features for reference */
    ...
    /* Print header */
    printf("# w1_57 w1_58 w1_59 sched_dW61 dg60_add dg60_xor pass\n");

    /* First: cert */
    {
        uint32_t w2_57 = cert_w1_57 + C57;
        uint32_t s1a[8], s2a[8];
        one_round(s1a, s1, cert_w1_57, 57);
        one_round(s2a, s2, w2_57, 57);
        uint32_t C58 = cascade_dw(s1a, s2a);
        uint32_t w2_58 = cert_w1_58 + C58;
        uint32_t s1b[8], s2b[8];
        one_round(s1b, s1a, cert_w1_58, 58);
        one_round(s2b, s2a, w2_58, 58);
        uint32_t C59 = cascade_dw(s1b, s2b);
        uint32_t w2_59 = cert_w1_59 + C59;
        uint32_t s1c[8], s2c[8];
        one_round(s1c, s1b, cert_w1_59, 59);
        one_round(s2c, s2b, w2_59, 59);
        uint32_t C60 = cascade_dw(s1c, s2c);
        uint32_t target_dW61 = sigma1(w2_59) - sigma1(cert_w1_59) + sched_const2_61 - sched_const1_61;

        /* Get state at round 60 with w1_60=0 (just to compute reference dg60) */
        uint32_t s1d[8], s2d[8];
        one_round(s1d, s1c, 0, 60);
        one_round(s2d, s2c, 0 + C60, 60);
        uint32_t dg60_add = s1d[6] - s2d[6];
        uint32_t dg60_xor = s1d[6] ^ s2d[6];

        printf("0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x CERT\n",
               cert_w1_57, cert_w1_58, cert_w1_59, target_dW61, dg60_add, dg60_xor);
    }
    ...
    for (long p = 0; p < n_prefixes; p++) {
        uint32_t w1_57 = (uint32_t)xorshift(&rng);
        uint32_t w1_58 = (uint32_t)xorshift(&rng);
        uint32_t w1_59 = (uint32_t)xorshift(&rng);
        ...
        /* Quick test */
        int pass = prefix_passes_round61(s1c, s2c, C60, target_dW61, 1L << 24);
        if (pass) n_pass++;

        printf("0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x %s\n",
               w1_57, w1_58, w1_59, target_dW61, dg60_add, dg60_xor, pass ? "PASS" : "fail");

        if (p % 100 == 99) {
            fprintf(stderr, "  [p=%ld, %lds] %ld passes so far\n", p+1, (long)(time(NULL)-t0), n_pass);
        }
    }

    fprintf(stderr, "\nTotal passes (out of %ld): %ld\n", n_prefixes, n_pass);
