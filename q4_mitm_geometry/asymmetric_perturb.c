/* Asymmetric prefix perturbation: vary ONE of W[57], W[58], W[59] at a time
 * over a small range, keeping other two at cert. Find which word's variation
 * preserves round-61 closure. */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
static long count_r61_matches(uint32_t w1_57, uint32_t w1_58, uint32_t w1_59,
...
int main(int argc, char **argv) {
    int which = argc > 1 ? atoi(argv[1]) : 59;  /* which word to perturb */
    ...
    fprintf(stderr, "Testing cert with single bit XOR perturbations of W[%d]:\n", which);
    int matches_count = 0;
    for (int bit = 0; bit < 32; bit++) {
        uint32_t w57 = cert_w1_57, w58 = cert_w1_58, w59 = cert_w1_59;
        if (which == 57) w57 ^= (1U << bit);
        else if (which == 58) w58 ^= (1U << bit);
        else if (which == 59) w59 ^= (1U << bit);

        long m = count_r61_matches(w57, w58, w59, s1, s2, C57, W1_pre, W2_pre);
        fprintf(stderr, "  bit %2d: %ld matches\n", bit, m);
        if (m > 0) matches_count++;
    }
    fprintf(stderr, "Bits that preserve round-61 closure: %d/32\n", matches_count);
    return 0;
}
