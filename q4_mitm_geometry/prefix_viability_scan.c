/* For random prefixes, compute the round-61 closure target dW61_target
 * and check if it lies in the cascade_dw image at round 61.
 *
 * This is the ACTUAL sr=60 viability test for a prefix:
 * "Is there any W[60] such that the cascade closes at round 61?"
 *
 * If yes, the prefix admits an sr=60 collision (with 8192-ish W[60] options
 * giving cascade_dw = target).
 *
 * Strategy:
 *   For each prefix:
 *     compute dW61_target = sigma1(W2[59]) - sigma1(W1[59]) + sched_const_diff
 *     compute image of cascade_dw at round 61 (2^32 enumeration)
 *     check if dW61_target ∈ image (constant time after enumeration)
 *
 * This costs ~7 sec/prefix on 24 cores.
 *
 * Test: 16 random prefixes. Goal: find ANY viable random prefix to expand
 * the known cert family.
 */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
/* For a given prefix:
 * - Build round-60 state s1c, s2c, C60
 * - Compute dW61_target
 * - Compute cascade_dw image at round 61, count distinct
 * - Check if dW61_target is in the image
 * Returns: 1 if viable (dW61_target in image), 0 otherwise.
 * Sets *count_out to image size, *match_count to number of W[60] giving target.
 */
static int test_prefix(uint32_t w1_57, uint32_t w1_58, uint32_t w1_59,
                        const uint32_t s1[8], const uint32_t s2[8],
                        uint32_t C57, const uint32_t W1_pre[64], const uint32_t W2_pre[64],
                        uint8_t *seen,
                        long *count_out, long *match_count_out) {
    ...
    /* Compute image and count matches against target */
    ...
    *match_count_out = matches;
    ...
    long distinct = 0;
    #pragma omp parallel for schedule(static) reduction(+:distinct)
    for (long i = 0; i < (1L << 32) / 8; i++) {
        distinct += __builtin_popcount(seen[i]);
    }
    *count_out = distinct;
...
int main(int argc, char **argv) {
    int n_prefixes = argc > 1 ? atoi(argv[1]) : 16;
    ...
    long c_count, c_matches;
    int viable = test_prefix(0x9ccfa55e, 0xd9d64416, 0x9e3ffb08,
                              s1, s2, C57, W1_pre, W2_pre, seen, &c_count, &c_matches);
    fprintf(stderr, "CERT: image=%ld, target_matches=%ld, viable=%s\n\n",
            c_count, c_matches, viable ? "YES" : "NO");
    if (!viable) {
        fprintf(stderr, "ERROR: Cert should be viable! Bug in test code.\n");
        free(seen);
        return 1;
    }
    ...
    for (int p = 0; p < n_prefixes; p++) {
        ...
        long count, matches;
        int v = test_prefix(w57, w58, w59, s1, s2, C57, W1_pre, W2_pre, seen, &count, &matches);
        time_t now = time(NULL);
        fprintf(stderr, "[%2d] W=0x%08x,0x%08x,0x%08x  image=%ld matches=%ld %s [%lds]\n",
                p, w57, w58, w59, count, matches, v ? "*** VIABLE ***" : "", (long)(now - t0));
        if (v) {
            n_viable++;
            fprintf(stderr, "    !!! Found viable random prefix !!!\n");
        }
        total_image += count;
    }

    fprintf(stderr, "\n=== Summary ===\n");
    fprintf(stderr, "Viable prefixes: %d / %d\n", n_viable, n_prefixes);
    fprintf(stderr, "Mean image size: %.0f (%.4f%% of 2^32)\n",
            (double)total_image / n_prefixes,
            100.0 * (double)total_image / n_prefixes / (double)(1L << 32));
    fprintf(stderr, "Expected viable rate (image/2^32): %.4f%%\n",
            100.0 * (double)total_image / n_prefixes / (double)(1L << 32));
