/*
 * cascade_chain_search.c — 4-level cascade chain search at N=32 (C version).
 *
 * Each iteration:
 * 1. Pick random W1[57]
 * 2. Compute W2[57] via cascade dW57
 * 3. Apply round 57, get state at round 57
 * 4. Compute dW58 from state57
 * 5. Pick random W1[58], compute W2[58]
 * 6. Apply round 58
 * 7. Repeat for W[59], W[60]
 * 8. Run remaining 3 rounds with schedule-derived W[61..63]
 * 9. Check collision HW at round 63
 *
 * Compile: gcc -O3 -march=native -fopenmp -o /tmp/casc_search q4_mitm_geometry/cascade_chain_search.c
 * Run: /tmp/casc_search [n_samples]
 */
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "4-level cascade chain search\n");
    fprintf(stderr, "M[0]=0x%08x, fill=0x%08x\n", m0, fill);
    ...
    #pragma omp parallel
    {
        ...
        int local_best = 256;
        uint32_t lb_w1_57 = 0, lb_w1_58 = 0, lb_w1_59 = 0, lb_w1_60 = 0;
        ...
        for (long iter = 0; iter < n_samples; iter++) {
            ...
            /* Now run W[61..63] schedule-determined */
    ...
    time_t t1 = time(NULL);
    long elapsed = t1 - t0;
    double rate = (double)total_evaluated / elapsed;

    fprintf(stderr, "\nDone in %lds\n", elapsed);
    fprintf(stderr, "Total samples: %ld\n", total_evaluated);
    fprintf(stderr, "Rate: %.0f samples/s\n", rate);
    ...
    printf("BEST_HW %d W1_57 0x%08x W1_58 0x%08x W1_59 0x%08x W1_60 0x%08x SAMPLES %ld\n",
           best_hw, best_w1_57, best_w1_58, best_w1_59, best_w1_60, total_evaluated);
